...
function getProducts(categoryId, parent) {
	$.ajax({
		url: "https://systemtrustng.herokuapp.com/products/" + categoryId, type: 'GET',
		success: function (response) {
			displayProducts(response.data, parent)
		},
...
function displayProducts(products, parent) {
	prods = products || []
	$.each(prods, function (ind, product) {
		console.log(product);
		var newchild = parent.find(".clone").clone();
		newchild.removeClass("clone");
		newchild.removeClass("hide");
		newchild.addClass("newchild");
		var name = product["name"]
		var id = product["productId"]
		if (name.length > 20) {
			name = product["name"].substring(0, 17) + "...";
		}
		var description = product["description"]
		if (description.length > 50) {
			description = product["description"].substring(0, 47) + "...";
		}
		var imagelink = product["images"][0];
		newchild.find(".product-image").attr("src", imagelink);
		newchild.find(".product-name").text(name);
		newchild.find(".product-id").text(id);
		newchild.find(".product-link").click(function () {
			localStorage.setItem("productId", id);
		});
		newchild.find(".product-price").text(product["price"].toLocaleString());
		newchild.find(".product-description").text(description);
...
function getProduct(productId) {
	$.ajax({
		url: "https://systemtrustng.herokuapp.com/product/" + productId, type: 'GET',
		success: function (response) {
...
function displayProduct(product) {
	...
	var imagelink = product["images"][0];
	parent.find(".prod-image").attr("src", imagelink);
	parent.find(".prod-name").text(name.toUpperCase());
	parent.find(".prod-cat").text(category.toUpperCase());
	parent.find(".prod-id").text(id);
	parent.find(".prod-price").text(product["price"].toLocaleString());
	parent.find(".prod-desc").text(description);
	parent.find(".prod-summary").text(summary);
